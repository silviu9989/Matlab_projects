...
[k1,Q1x,Q1y]=ECPM_cu_cheie(Gx,Gy,p);
[k2,R1x,R1y]=ECPM_cu_cheie(Q1x,Q1y,p);
[Q2x,Q2y]=ECPM_fara_cheie(Gx,Gy,k2,p);
[R2x,R2y]=ECPM_fara_cheie(Q2x,Q2y,k1,p);
...
nr_unu_cheie=20;%exceptand prima si ultima pozitie
vec=unique(round(252*rand(1,20))+2);
while length(vec)< 20
    vec=unique(round(253*rand(1,nr_unu_cheie))+1);
end
vec(nr_unu_cheie+1)=0;
vec(nr_unu_cheie+2)=255;
k1=sum(vpi(2).^vec);
